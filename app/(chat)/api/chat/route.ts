...
// Dynamic import for resumable-stream to avoid Redis connection during module load
type ResumableStreamContext = any;
...
export async function getStreamContext() {
  // Check if Redis URL is available before creating resumable stream context
  ...
  if (!globalStreamContext) {
    try {
      // Dynamic import to avoid Redis connection during module load
      ...
      console.log(
        ' > Resumable streams are disabled due to Redis connection error:',
        error.message,
      );
      return null;
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Fetch user data for personalized prompts
    let userData = null;
    try {
      ...
      if (users.length > 0) {
        ...
        userData = {
          ...
          peakMoment: user.peakMoment || undefined,
          values: user.values as string[] | undefined,
          constraints: user.constraints,
          immediateGoal: user.immediateGoal || undefined,
    ...
    const stream = createUIMessageStream({
      execute: ({ writer: dataStream }) => {
        const result = streamText({
          ...
          system: systemPrompt({ selectedChatModel, requestHints, userData: userData || undefined, domain }),
